...
const REVIEWS = [
  { id: 'r-1', author: 'Piotr i Anna Kamińscy', score: 5, date: '2026-04-27', text: 'Trzeci raz w tym apartamencie i jak zawsze – perfekcyjnie. Polecamy serdecznie!',           published: true  },
  { id: 'r-2', author: 'Carlos García',          score: 5, date: '2026-04-13', text: 'Perfecto apartamento, la ubicación es inmejorable y el anfitrión muy atento.',              published: true  },
  { id: 'r-3', author: 'Sophie Martin',          score: 5, date: '2026-05-11', text: 'Appartement magnifique avec une terrasse splendide. Nous reviendrons!',                       published: false },
]
...
function StatusBadge({ status }: { status: BookingStatus }) {
  ...
  return (
    <span style={{ fontSize: '0.72rem', fontWeight: 700, padding: '0.22rem 0.6rem', borderRadius: 20, whiteSpace: 'nowrap', background: bg, color }}>
      {label}
    </span>
  )
}
...
function DashboardView({ setTab, setBookingId }: { setTab: (t: Tab) => void; setBookingId: (id: string) => void }) {
  ...
  const upcoming = BOOKINGS.filter(b => ['confirmed', 'awaiting_payment', 'checked_in'].includes(b.status))

  return (
    <div>
      <h1 style={{ fontSize: '1.3rem', fontWeight: 800, color: '#111827', marginBottom: '1.5rem' }}>Dashboard</h1>

      {/* Stats — always auto-fit, min 160px so 2 per row on small mobile */}
          ...
          {upcoming.map(b => (
            <div key={b.id} onClick={() => { setBookingId(b.id); setTab('bookings') }}
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.name}</div>
                <div style={{ fontSize: '0.73rem', color: '#9CA3AF' }}>{b.arrival} → {b.departure}</div>
              </div>
              ...
              {list.map((b, i) => (
                ...
                >
                  <td style={{ padding: '0.85rem 1rem', maxWidth: 160 }}>
                    <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.name}</div>
                    <div style={{ fontSize: '0.72rem', color: '#9CA3AF' }}>{b.country}</div>
                  </td>
                  ...
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.83rem', color: b.deposit > 0 ? '#15803D' : '#9CA3AF', whiteSpace: 'nowrap' }}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h1 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#111827', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.name}</h1>
          <div style={{ fontSize: '0.75rem', color: '#9CA3AF', marginTop: 2 }}>Rezerwacja {b.id} · {new Date(b.created_at).toLocaleDateString('pl-PL')}</div>
        </div>
        <div style={{ flexShrink: 0 }}><StatusBadge status={b.status} /></div>
      </div>

      {/* Top 3 cards — auto-fit, 1 col on narrow mobile */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <Card>
          ...
          <div style={{ marginBottom: '0.5rem' }}>
            <div style={{ fontSize: '0.7rem', color: '#9CA3AF', marginBottom: '0.3rem' }}>Status</div>
      ...
      {checkin ? (
        ...
        <Card style={{ marginBottom: '1.5rem', borderLeft: '4px solid #E5E7EB' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => demoAlert('Link do formularza check-in zostałby wysłany.')} style={{ background: PRIMARY, color: 'white', border: 'none', borderRadius: 8, padding: '0.45rem 0.9rem', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>
              Wyślij link
            </button>
          </div>
        </Card>
      ...
      {/* Chat + emails — 1-col on mobile */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1.5rem' }}>
        <Card>
          <SectionTitle>💬 Wiadomości</SectionTitle>
          <div style={{ height: 260, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.6rem', marginBottom: '0.75rem', paddingRight: '0.25rem' }}>
            {messages.length === 0 && <p style={{ color: '#9CA3AF', fontSize: '0.82rem', margin: 0 }}>Brak wiadomości.</p>}
            {messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: m.side === 'host' ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  maxWidth: '80%', padding: '0.5rem 0.75rem',
                  borderRadius: m.side === 'host' ? '12px 12px 2px 12px' : '12px 12px 12px 2px',
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <input value={chatMsg} onChange={e => setChatMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMsg()}
              ...
              style={{ flex: 1, border: `1px solid ${CARD_BD}`, borderRadius: 8, padding: '0.45rem 0.7rem', fontSize: '0.83rem', fontFamily: 'inherit', outline: 'none', minWidth: 0 }}
            />
            <button onClick={sendMsg} style={{ background: PRIMARY, color: 'white', border: 'none', borderRadius: 8, padding: '0.45rem 0.9rem', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>
              Wyślij
            </button>
          </div>
        </Card>
        ...
        <Card>
          ...
          {emails.length === 0 && <p style={{ color: '#9CA3AF', fontSize: '0.82rem', margin: 0 }}>Brak emaili.</p>}
          {emails.map((em, i) => (
            <div key={i} style={{ borderBottom: i < emails.length - 1 ? `1px solid #F3F4F6` : 'none', paddingBottom: '0.6rem', marginBottom: '0.6rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: '0.82rem', fontWeight: 600, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{em.subject}</div>
                  <div style={{ fontSize: '0.7rem', color: '#9CA3AF', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{em.from} · {em.date}</div>
                </div>
                <button onClick={() => setExpandedEmail(expandedEmail === `${i}` ? null : `${i}`)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', color: PRIMARY, fontFamily: 'inherit', whiteSpace: 'nowrap', padding: 0, flexShrink: 0 }}>
                  {expandedEmail === `${i}` ? '▲' : '▼'}
                </button>
              </div>
              {expandedEmail === `${i}` && (
            ...
            <tbody>
              {BOOKINGS.map((b, i) => (
                <tr key={b.id}
                  ...
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'white' }}
                >
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.85rem', fontWeight: 600, color: '#111827', whiteSpace: 'nowrap' }}>{b.name}</td>
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: '#374151' }}>{b.email}</td>
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: '#374151', whiteSpace: 'nowrap' }}>{b.phone ?? '—'}</td>
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: '#374151', whiteSpace: 'nowrap' }}>{b.country}</td>
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: '#374151', textAlign: 'center' }}>1</td>
                  <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: '#374151', whiteSpace: 'nowrap' }}>{b.departure}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <Card>
          ...
          <p style={{ fontSize: '0.82rem', color: '#9CA3AF', marginBottom: '0.75rem', marginTop: 0 }}>Podaj daty (RRRR-MM-DD), kiedy apartament jest niedostępny (np. własne pobyty).</p>
      ...
      <Card style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(2px)' }}>
          ...
          <button onClick={() => demoAlert('Przekierowanie do nobooking.eu/cennik')} style={{ marginTop: '0.75rem', background: GOLD, color: 'white', border: 'none', borderRadius: 8, padding: '0.5rem 1.25rem', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            Przejdź na PRO →
          </button>
        </div>
      </Card>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1.5rem' }}>
        ...
        <Card>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: `1px solid #F3F4F6`, gap: '0.75rem' }}>
            <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: `1px solid #F3F4F6`, gap: '0.75rem' }}>
            ...
            <button onClick={() => demoAlert('SMS dostępne w planie PRO.')} style={{
              width: 44, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer', padding: 0, flexShrink: 0,
              background: smsEnabled ? '#15803D' : '#D1D5DB', position: 'relative', opacity: 0.5,
            }}>
          ...
          <div style={{ background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: 10, padding: '0.75rem', marginTop: '0.25rem' }}>
            ...
            <button onClick={() => demoAlert('Konfiguracja Stripe.')} style={{ marginTop: '0.5rem', background: GOLD, color: 'white', border: 'none', borderRadius: 7, padding: '0.35rem 0.75rem', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              Połącz Stripe
            </button>
          </div>
        </Card>
...
export default function AdminDemoPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: PAGE_BG, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      {/* Demo banner — NOT sticky, just sits at top and scrolls away */}
      <div style={{ background: '#7C3AED', color: 'white', padding: '0.55rem 1rem', fontSize: '0.78rem', fontWeight: 600, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
        <span>{isMobile ? '👁 Demo panelu — dane przykładowe' : '👁 Demo panelu właściciela — dane przykładowe, przyciski nie wykonują akcji'}</span>
        <a href="https://www.nobooking.eu/#cennik" target="_blank" rel="noopener noreferrer" style={{ color: 'white', fontWeight: 800, textDecoration: 'underline', whiteSpace: 'nowrap', fontSize: '0.78rem' }}>
          Zamów własny →
        </a>
      </div>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 38px)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          {/* Top bar — sticky at top:0 (banner scrolls away) */}
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
              ...
              <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>🏠 Casa Sol Torrevieja</div>
            </div>
